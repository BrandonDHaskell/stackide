...
These are open and must be settled inside the milestone, not deferred past it.

1. **Shader compilation toolchain.** `glslang` is the default assumption. It is
   not currently in the toolchain container and must be added
   (`glslang-tools` on Debian) with a pinned version.
2. **Cross-compilation for Metal.** SPIR-V alone does not serve macOS. Either
   take a dependency on SDL_shadercross, or defer and accept that macOS support
   means writing MSL separately later. Decide, record it, do not leave it
   implicit.
3. **Swapchain colorspace.** Whether the swapchain format is sRGB changes the
   float-to-byte mapping of every clear and every sampled texel, and the
   offscreen pixel assertions depend on it. Pick one, state it, and make the
   offscreen target match the swapchain.
4. **Shader embedding mechanism.** A CMake-generated byte-array header now.
   Revisit `#embed` once both GCC 16 and Clang support it in C++ mode.
